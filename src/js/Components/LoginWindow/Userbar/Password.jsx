...
class Password extends React.Component {
	constructor(props) {
		...
		if (window.__is_debug != true) {
			this.auth_event = () => {
				lightdm.respond(this.state.value);
				setTimeout(() => {
	...
	componentDidMount() {
		if (window.__is_debug != true) {
			lightdm.show_prompt.connect(this.auth_event);
		}
	...
	componentWillUnmount() {
		if (window.__is_debug != true) {
			lightdm.show_prompt.disconnect(this.auth_event);
		}
		if (this.focusTimer) {
			...
			if (e.which == 13) {
				...
				if (window.__is_debug === true) {
					if (this.state.value == "password") {
						notify(
							data.get(this.props.lang, "notifications.logged_in") +
								" " +
						...
						notify(
							data.get(this.props.lang, "notifications.wrong_password"),
							types.Error,
						);
						this.props.failure();
					}
					...
					lightdm.authenticate(this.props.user.username);
				}
			} else if (e.which == 8 || e.which == 46) {
				//Backspace key or Supr key
		...
		return (
			<div
				...
				className={cxs({
					...
					minHeight:
						"calc(1.2em + " +
						+this.props.border.top.slice(0, 1) * 2 +
						"px + 1px)",
					position: "relative",
					"::before": {
						content: this.state.active
							? '"' + this.props.caret.left + '"'
							: '""',
					},
					"::after": {
						content: this.state.active
							? '"' + this.props.caret.right + '"'
							: '""',
					},
				})}
			>
