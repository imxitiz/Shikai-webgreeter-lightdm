...
export default function ModernStyleTab() {
  ...
  const currentLogoName =
    style.sidebar?.logo?.split('/').pop()?.replace(/\.[^/.]+$/, '') || ''
    ...
    <div className="space-y-4 pb-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-4 rounded-xl bg-input/30 border border-border/30">
            <div className="w-16 h-16 rounded-lg bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center shadow-sm">
              {style.sidebar?.logo ? (
                <img src={style.sidebar.logo} alt="Logo" className="w-12 h-12 object-contain" />
              ) : (
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.text') || 'Text Color'}
            value={style.main?.textcolor || '#ffffff'}
            onChange={(c) => set('main.textcolor', c)}
          />
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.avatar') || 'Avatar Color'}
        ...
        <div className="grid grid-cols-2 gap-4">
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.sidebar') || 'Sidebar'}
            value={style.sidebar?.background || '#1a1a1a'}
            onChange={(c) => set('sidebar.background', c)}
          />
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.userbar_top') || 'Panel Top'}
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.userbar_bottom') || 'Panel Bottom'}
            value={style.userbar?.background?.bottom || '#0a0a0a'}
            onChange={(c) => set('userbar.background.bottom', c)}
          />
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.icon_background') || 'Icon Background'}
        ...
        <div className="grid grid-cols-2 gap-4">
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.session_text') || 'Session Text'}
            value={style.userbar?.session?.color || '#ffffff'}
            onChange={(c) => set('userbar.session.color', c)}
          />
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.session_background') || 'Session Background'}
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.password_text') || 'Password Text'}
            value={style.userbar?.password?.color || '#ffffff'}
            onChange={(c) => set('userbar.password.color', c)}
          />
          <ColorPicker
            label={data.get(lang, 'settings.style.sections.main.options.password_background') || 'Password Background'}
        ...
        <div className="space-y-4">
          <div>
            <Label className="text-xs text-muted-foreground mb-2 block">
              {data.get(lang, 'settings.style.sections.misc.options.password') || 'Password Border Radius'}
            </Label>
            <Input
          ...
          <div>
            <Label className="text-xs text-muted-foreground mb-2 block">
              {data.get(lang, 'settings.style.sections.misc.options.session') || 'Session Border Radius'}
            </Label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label className="text-xs text-muted-foreground mb-2 block">
                {data.get(lang, 'settings.style.sections.misc.options.caret.left') || 'Caret Left'}
              </Label>
              <Input
            ...
            <div>
              <Label className="text-xs text-muted-foreground mb-2 block">
                {data.get(lang, 'settings.style.sections.misc.options.caret.right') || 'Caret Right'}
              </Label>
              <Input
